...
class User(db.Model):
    ...
    costs = db.relationship("Cost", backref="user", lazy="dynamic")
    __tablename__ = 'users'
    id = db.Column(db.Integer, primary_key=True)
...
class Sale(db.Model):
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=True)  # Added user reference
    payment_method = db.Column(db.String(20), nullable=False, default="Gotówka")  # Nowe pole
    __tablename__ = 'sales'
    id = db.Column(db.Integer, primary_key=True)
