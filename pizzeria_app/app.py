...
def add_cost():
    if request.method == 'POST':
        ...
        payment_method = request.form['payment_method']  # Get the payment method (e.g., "Gotówka")

        new_cost = Cost(date=cost_date, category=category, description=description, amount=amount, payment_method=payment_method)
        db.session.add(new_cost)

        # Update the safe balance if payment method is cash
...
@login_required
def safe():
    from models import Sale, Cost
    safe_balance = 0

    # Adding cash from sales
    sales = Sale.query.all()
    for sale in sales:
        if sale.payment_method == 'Gotówka':
            safe_balance += sale.amount

    # Subtracting cash from costs
    costs = Cost.query.all()
    for cost in costs:
        if cost.payment_method == 'Gotówka':
            safe_balance -= cost.amount

    return render_template('safe.html', balance=safe_balance)
