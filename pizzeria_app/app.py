...
@login_required
def finance_status():
    # Oblicz przychody i wydatki
    total_income = db.session.query(db.func.sum(Sale.amount)).scalar() or 0
    total_expenses = db.session.query(db.func.sum(Cost.amount)).scalar() or 0
...
def cost_summary():
    cost_summary = (
        db.session.query(Cost.category, db.func.sum(Cost.amount))
        .group_by(Cost.category)
        .all()
    )
    cost_summary = {category: total for category, total in cost_summary}
