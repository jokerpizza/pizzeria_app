...
def safe():
    ...
    total_cash_in = sum(s.dine_in for s in sales)  # Wszystkie wpłaty gotówkowe ze sprzedaży
    ...
    recent_transactions = (
        [(s.date, "Sprzedaż", s.dine_in) for s in sales if s.dine_in > 0] +
...
class SafeTransaction(db.Model):
    ...
    date = db.Column(db.String(10), nullable=False)  # Data transakcji
    type = db.Column(db.String(20), nullable=False)  # 'Wpłata' lub 'Wypłata'
    amount = db.Column(db.Float, nullable=False)  # Kwota transakcji
