...
def safe():
    ...
    if request.method == 'POST':
        try:
            ...
            if deposit_amount > 0 and source_type in ['karta', 'przelew', 'zaplacono']:
                ...
                if sale:
                    ...
                    db.session.commit()  # Aktualizacja istniejącego rekordu
    ...
    # Tworzenie zakresu dat dla filtrowania (uwzględnia dzień!)
    start_date = f"{selected_year}-{selected_month:02d}-01"
    end_date = (datetime.strptime(start_date, "%Y-%m-%d") + timedelta(days=31)).strftime("%Y-%m-%d")
