...
def safe():
    ...
    if request.method == 'POST':
        ...
        if deposit_amount > 0 and source_type in ['karta', 'przelew', 'zaplacone']:
            # Odjęcie z wybranego źródła
            if source_type == 'karta':
                Sale.query.filter(Sale.date >= today.strftime("%Y-%m-%d")).update({'przelew': Sale.przelew - deposit_amount})
            elif source_type == 'przelew':
                Sale.query.filter(Sale.date >= today.strftime("%Y-%m-%d")).update({'przelew': Sale.przelew - deposit_amount})
            elif source_type == 'zaplacone':
                Sale.query.filter(Sale.date >= today.strftime("%Y-%m-%d")).update({'zaplacono': Sale.zaplacono - deposit_amount})

            # Dodanie wpłaty do sejfu
            new_safe_transaction = Cost(
                date=today.strftime("%Y-%m-%d"),
                category="Wpłata do sejfu",
                description="Gotówka z bankomatu/karty/przelewu",
                amount=-deposit_amount,  # Negatywna wartość, bo dodajemy do sejfu
    ...
    costs_query = Cost.query.filter(Cost.date >= start_date, Cost.date < end_date, Cost.payment_method == "Gotówka")

    if selected_day != "all":
        day_str = f"{selected_year}-{selected_month:02d}-{int(selected_day):02d}"
        sales_query = sales_query.filter(Sale.date == day_str)
        costs_query = costs_query.filter(Cost.date == day_str)

    sales = sales_query.all()
    costs = costs_query.all()

    # Obliczenie aktualnego stanu gotówki w sejfie
    total_cash_in = sum(s.gotowka for s in sales) if sales else 0
    total_cash_out = sum(c.amount for c in costs) if costs else 0
    current_safe_balance = total_cash_in - total_cash_out

    # Pobranie ostatnich 10 transakcji z wybranego okresu
    recent_transactions = (
        [(s.date, "Sprzedaż", s.gotowka) for s in sales if s.gotowka > 0] +
        [(c.date, "Koszt", -c.amount) for c in costs]
    )
    recent_transactions.sort(reverse=True, key=lambda x: x[0])
    ...
    recent_transactions.sort(reverse=True, key=lambda x: x[0])  # Sortowanie po dacie malejąco
