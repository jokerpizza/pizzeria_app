...
@app.route('/sejf', methods=['GET'])
@login_required
def sejf():
    # Calculate current cash balance
    sales = Sale.query.filter_by(payment_method='Gotówka').all()
    costs = Cost.query.filter_by(payment_method='Gotówka').all()

    cash_balance = sum(s.dine_in for s in sales) - sum(c.amount for c in costs)
    ...
    for sale in sales:
        logs.append({
            ...
            'operation_type': 'Sprzedaż',
            'amount': sale.dine_in
        })
    for cost in costs:
        logs.append({
            'date': cost.date,
            'user': cost.user.username,
            'operation_type': 'Koszt',
            'amount': -cost.amount
        })
    ...
    return render_template('sejf.html', cash_balance=cash_balance, logs=logs)
...
def update_database_schema():
    """Aktualizacja schematu bazy danych dla SQLite."""
    with app.app_context():
        try:
            with db.engine.connect() as connection:
                # Check if 'user_id' exists in 'sales' table
                ...
                # Check if 'user_id' exists in 'costs' table
                result = connection.execute(text("PRAGMA table_info(costs);"))
                columns = [row[1] for row in result.fetchall()]
                if "user_id" not in columns:
                    connection.execute(text("ALTER TABLE costs ADD COLUMN user_id INTEGER;"))

        except Exception as e:
            import traceback
            logger.error(f"Błąd przy aktualizacji schematu bazy danych: {e}")
            logger.error(traceback.format_exc())

    """Update database schema manually if columns are missing."""
    with app.app_context():
        try:
            # Add user_id column to 'sales' table if it does not exist
            with db.engine.connect() as connection:
    ...
    if "user_id" not in columns:
        connection.execute(text("ALTER TABLE sales ADD COLUMN user_id INTEGER;"))
            ...
            # Add user_id column to 'costs' table if it does not exist
            with db.engine.connect() as connection:
