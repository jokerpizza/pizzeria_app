...
def safe():
    # Pobranie wszystkich transakcji sprzedaży, kosztów i sejfu
    sales = Sale.query.all()
    costs = Cost.query.filter_by(payment_method="Gotówka").all()
    manual_transactions = SafeTransaction.query.all()
    ...
    total_manual = sum(t.amount for t in manual_transactions)  # Wszystkie ręczne operacje

    current_safe_balance = total_cash_in - total_cash_out + total_manual  # Aktualny stan sejfu
    ...
    recent_transactions = (
        ...
        [(t.date, "Ręczna Wpłata" if t.amount > 0 else "Ręczna Wypłata", t.amount) for t in manual_transactions]
    )
    recent_transactions.sort(reverse=True, key=lambda x: datetime.strptime(x[0], '%Y-%m-%d'))  # Sortowanie według daty malejąco
...
def withdraw_safe():
    ...
    if amount > 0:
        new_transaction = SafeTransaction(date=date, amount=-amount, description=description)
        db.session.add(new_transaction)
        db.session.commit()  # Upewnienie się, że transakcja zostaje zapisana

    return redirect(url_for('safe'))



# Aktualizacja funkcji safe() do uwzględnienia ręcznych transakcji
...
def safe():
    ...
    return render_template(
        ...
        print(f"Error in deposit_safe: {e}")
        return redirect(url_for('safe'))


    except Exception as e:
        print(f"Error in withdraw_safe: {e}")
        return redirect(url_for('safe'))


@app.route('/deposit_safe', methods=['POST'])
@login_required
def deposit_safe():
...
@app.route('/withdraw_safe', methods=['POST'])
@login_required
def withdraw_safe():
    amount = float(request.form.get('amount', 0))
    description = request.form.get('description', 'Brak opisu')
