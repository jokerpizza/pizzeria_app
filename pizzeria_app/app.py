...
def finance_status():
    ...
    monthly_sales = sum(s.dine_in + s.delivery + s.other for s in sales if s.date.startswith(current_year_month))
    monthly_costs = sum(c.amount for c in costs if c.date.startswith(current_year_month))
