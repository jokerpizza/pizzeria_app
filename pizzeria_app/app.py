...
from models import db, User, Sale, Cost, CostCategory
...
def add_cost():
    if request.method == 'POST':
        ...
        new_cost = Cost(date=cost_date, category=category, description=description, amount=amount, payment_method=request.form['payment_method'])
...
def settings():
    if request.method == 'POST':
        ...
        if new_category_name:
            existing_category = CostCategory.query.filter_by(name=new_category_name).first()
            if not existing_category:
                new_category = CostCategory(name=new_category_name)
                db.session.add(new_category)
                db.session.commit()
        # Delete category
        delete_category_id = request.form.get('delete_category_id')
        if delete_category_id:
            category_to_delete = CostCategory.query.get(delete_category_id)
