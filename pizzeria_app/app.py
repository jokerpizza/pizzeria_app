...
@login_required
def safe():
    # Pobranie parametrów filtrów daty
    start_date = request.args.get("start_date")
    end_date = request.args.get("end_date")

    # Pobranie transakcji sprzedaży i kosztów gotówkowych
    sales = Sale.query.all()
    costs = Cost.query.filter_by(payment_method="Gotówka").all()
    atm_deposits = ATMDeposit.query.all()

    # Filtracja transakcji po dacie, jeśli użytkownik podał zakres
    ...
    # Sortowanie i przekazanie danych do widoku
...
def deposit_atm():
    ...
    if amount:
        amount = float(amount)
        new_deposit = ATMDeposit(amount=amount)
