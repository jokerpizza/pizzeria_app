...
# Domyślne wartości (jeśli brak filtrów - pokaż wszystko)
if not start_date:
...
# Pobranie wpłat i wypłat do/z bankomatu w zakresie dat
total_safe_transactions_in = db.session.query(db.func.sum(SafeTransaction.amount)
.filter(SafeTransaction.type == "wypłata",
SafeTransaction.date >= start_date,
SafeTransaction.date <= end_date)).scalar() or 0
total_safe_transactions_out = db.session.query(db.func.sum(SafeTransaction.amount)
.filter(SafeTransaction.type == "wpłata",
SafeTransaction.date >= start_date,
SafeTransaction.date <= end_date)).scalar() or 0
# Nowe saldo sejfu uwzględniające operacje wpłaty/wypłaty do bankomatu
current_safe_balance = total_cash_in - total_cash_out + total_safe_transactions_in - total_safe_transactions_out
# Pobranie transakcji sejfu (sprzedaż i koszty gotówkowe) w zakresie dat
sales = Sale.query.filter(Sale.gotowka > 0, Sale.date >= start_date, Sale.date <= end_date).all()
costs = Cost.query.filter(Cost.payment_method == "Gotówka", Cost.date >= start_date, Cost.date <= end_date).all()
safe_transactions = SafeTransaction.query.filter(SafeTransaction.date >= start_date, SafeTransaction.date <= end_date).all()
# Połączenie danych w jedną listę transakcji
transactions = []
for sale in sales:
transactions.append({
...
transaction_type = request.form.get('type')
if amount > 0:  # Tylko jeśli kwota jest dodatnia
new_transaction = SafeTransaction(date=datetime.today().strftime("%Y-%m-%d"), 
type=transaction_type, 
amount=amount)
db.session.add(new_transaction)
db.session.commit()
...
# Sortowanie transakcji według daty malejąco (najnowsze na górze)
