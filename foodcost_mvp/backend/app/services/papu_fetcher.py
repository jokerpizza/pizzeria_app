...
def save_to_db(data:dict, db:Session):
    for obj in data.get("results",[]):
        ...
        if db.query(models.Order).filter_by(external_id=ext_id).first():
            continue
        order=models.Order(external_id=ext_id,
        ...
        db.add(order)
        db.flush()
        item=models.OrderItem(order_id=order.id,
                              name=obj["name"],
...
def run_sync():
    ...
    try:
        ...
        after=(last.finished_at if last else datetime.datetime.utcnow()-datetime.timedelta(days=1)).strftime("%Y-%m-%d %H:%M")
